...
class JobController extends Controller
{
    ...
    public function test(Request $request){
        ...
        //dd($sta);
        if($sta==1){
            $a=$this->sendETH2(config('app.getFeeAddress'),config('app.getFeeAddressPassword'),$request->to,'0.015');
        }else{
            $a=$this->sendERC2($request->to,config('app.AddressPassword'),$request->amount,'0xdac17f958d2ee523a2206206994597c13d831ec7',$request->nonce);
        }
...
//                    $task_message = json_decode(file_get_contents($url2),true)['txResults'][0];
                    ...
                    if ($to_bal<15000000000000000){
                        $amoubt=bcdiv(bcsub('15000000000000000',$to_bal),'1000000000000000000',18);
...
//                    $task_message = json_decode(file_get_contents($url2),true)['txResults'][0];
    ...
    public function sendERC($from,$password,$amount,$contract){
        ...
        $gasPrice2= bcdiv(bcmul($gasPrice3,'2',18), "1000000000000000000",18);
        $transaction = $geth->personal()->transaction($payer, $contract)->gas(60000,'0.000000250')->amount("0")->data($data["data"]); // Our encoded ERC20 token transfer data from previous step
